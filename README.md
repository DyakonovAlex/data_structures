...
### Асимптотика по памяти (Big O) для `UnsortedArray`

- **Память структуры**: \(O(n)\), где \(n = capacity\).
  - Хранится фиксированный буфер `Box<[Option<T>]>` длины `capacity`.
  - Дополнительно — поле `size` (\(O(1)\)).
- **Дополнительная память операций**:
  - **`insert` / `delete` / `get` / `size` / `capacity` / `is_empty` / `is_full`**: \(O(1)\) дополнительной памяти.
  - **`find` / `traverse`**: \(O(1)\) дополнительной памяти (один проход, без выделений).
